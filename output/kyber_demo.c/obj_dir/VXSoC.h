...
#ifndef VERILATED_VXSOC_H_
#define VERILATED_VXSOC_H_  // guard

#include "verilated.h"

class VXSoC__Syms;
class VXSoC___024root;
class VerilatedVcdC;

// This class is the main interface to the Verilated model
class alignas(VL_CACHE_LINE_BYTES) VXSoC VL_NOT_FINAL : public VerilatedModel {
  private:
    // Symbol table holding complete model state (owned by this class)
    VXSoC__Syms* const vlSymsp;
  ...
  public:
    ...
    explicit VXSoC(VerilatedContext* contextp, const char* name = "TOP");
    explicit VXSoC(const char* name = "TOP");
    /// Destroy the model; called (often implicitly) by application code
    virtual ~VXSoC();
  private:
    VL_UNCOPYABLE(VXSoC);  ///< Copying not allowed
